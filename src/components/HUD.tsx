...
export default function HUD({ 
    ...
    useEffect(() => {
        if (clickCount >= 5 && !isGlitchingExternal) {
            onGlitchToggle(true);
    ...
    }, [clickCount, isGlitchingExternal, onGlitchToggle]);

    useEffect(() => {
        let interval: number;
        if (isGlitchingExternal) {
        ...
        <>
            ...
            <AnimatePresence>
                {isGlitchingExternal && (
                    ...
                    >
                        {/* NOISE OVERLAY - NO BLACK BG */}
                        <div className="absolute inset-0 bg-red-600/5 backdrop-blur-[2px]"></div>
                        ...
                        >
                            <h1 
                                className="text-white text-5xl md:text-8xl font-black italic tracking-tighter leading-none"
                                style={{ 
                                    textShadow: '4px 4px 0px #ff0000, -4px -4px 0px #00ffff',
                            ...
                            {/* GHOST LAYERS */}
                            <motion.h1 
                                animate={{ x: [-10, 10, -10], opacity: [0.5, 0.2, 0.5] }}
                                transition={{ repeat: Infinity, duration: 0.1 }}
                                className="absolute inset-0 text-cyan-400 text-5xl md:text-8xl font-black italic tracking-tighter leading-none mix-blend-screen opacity-50 blur-sm"
                            >
                                {glitchWords[wordIndex]}
                            </motion.h1>
                            <motion.h1 
                                ...
                                className="absolute inset-0 text-red-600 text-5xl md:text-8xl font-black italic tracking-tighter leading-none mix-blend-screen opacity-50 blur-sm"
                            >
                                {glitchWords[wordIndex]}
                            </motion.h1>
                        </motion.div>
